...
void BlockDown(int sig){
	...
	else if (blockY==-1) // 게임오버조건 -> 블록이 천장에 닿음  
	{
		...
		for (i=3; i>=0; i--)
		{
				if (block[nextBlock[0]][blockRotate][3][i]==1)
				{
					temp=1;			
				} 
		...
		if (temp==0) AddBlockToField(field, nextBlock[0],blockRotate, blockY, blockX);
		else AddBlockToField(field, nextBlock[0], blockRotate, blockY-i, blockX);
		DrawField();
		for (i=0; i<WIDTH;i++)
			if (field[1][i]==1) {
				check=1;
